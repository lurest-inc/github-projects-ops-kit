...
| エラー | 原因 | 対処 |
|--------|------|------|
| `Bad credentials (HTTP 401)` | PAT が期限切れまたは無効化されている | GitHub Settings → Developer settings → Personal access tokens でトークンを再発行し、Repository Secrets の `PROJECT_PAT` を更新する |

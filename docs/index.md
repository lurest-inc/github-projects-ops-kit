...
| リポジトリにラベルを一括追加したい | [⑤ Issue ラベル一括追加](workflows/05-setup-repository-labels) | 設定ファイルで定義したラベルをリポジトリに一括作成 |
| 放置されたタスクを検知したい | [⑥ 滞留アイテム検知](workflows/06-detect-stale-items) | 一定期間更新がない `Project` アイテムを検知・レポート |
| プロジェクトの状況を把握したい | [⑧ プロジェクトサマリーレポート](workflows/08-generate-summary-report) | `Project` のアイテムをステータス別・担当者別・ラベル別に集計・レポート |
| 工数を集計・分析したい | [⑨ 工数集計レポート](workflows/09-generate-effort-report) | 見積もり工数・実績工数を多角的に集計・分析し工数管理を支援 |

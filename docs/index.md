...
| Issue/PR をまとめて取り込みたい | [⑤ Issue/PR 一括紐付け](workflows/05-add-items-to-project) | Project に Repository の Issue/PR を一括追加 |
...
| 滞留タスクを検知したい | [⑥ 統合 Project 分析](workflows/06-analyze-project) | 指定日数以上動きのないアイテムを自動検出（`report_types: stale`） |
| サマリーレポートを出したい | [⑥ 統合 Project 分析](workflows/06-analyze-project) | Status 別・担当者別の集計レポートを生成（`report_types: summary`） |
| 工数・ベロシティを把握したい | [⑥ 統合 Project 分析](workflows/06-analyze-project) | 工数集計・ベロシティレポートを生成（`report_types: effort,velocity`） |

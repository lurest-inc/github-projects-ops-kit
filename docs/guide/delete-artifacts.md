...
Workflow で生成されたArtifactを手動で削除する方法を説明します。

> **Note:** Artifactの公開範囲に関する注意事項は [⑤ 統合プロジェクト分析](../workflows/05-analyze-project.md#️-Artifactの公開範囲に関する注意事項) を参照してください。
...
- 一度削除したArtifactは **復元できません**
- 削除にはRepository への **Write（書き込み）アクセス** が必要です
...
5. 削除したいArtifactの横にある **🗑️（ゴミ箱アイコン）** をクリックする
6. 確認ダイアログで削除を承認する
...
Workflow 実行を削除すると、紐づくArtifactも全て削除されます。

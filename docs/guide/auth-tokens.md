...
<br><br>
<strong>Note:</strong> release-please ワークフローでは GitHub Release の作成やタグの書き込みを行うため、<code>repo</code> スコープが必要です。<code>public_repo</code> では Contents や Pull requests の書き込み権限が含まれないため、release-please は動作しません。
</blockquote>

...
指定した GitHub `Project` のItemを走査し、滞留Item検知・プロジェクトサマリーレポート・工数集計レポート・ベロシティレポート・Itemエクスポートを 1 回の実行でまとめて生成します。`report_types` パラメータで実行する機能を選択することも可能です。
...
| `item_type` | 対象Itemの種別 | ✅ | `choice` | `all` | `issues` |
| `item_state` | 対象Itemの状態 | ✅ | `choice` | `all` | `open` |
| `retention_days` | Artifactの保持日数（1〜7） | ✅ | `number` | `7` | `3` |
...
|------|------|
| `all` | 全状態のItemを対象 |
| `open` | Open 状態のItemのみを対象 |
| `closed` | Closed / Merged 状態のItemのみを対象 |

---

## 🔍 滞留Item検知（stale）

一定期間更新がない「滞留Item」を検知・レポートします。
...
| 条件 | 理由 |
|------|------|
| Statusが `Done` | 完了済みのため検知不要 |
| Statusが `Backlog` | 未着手のバックログは滞留とみなさない |
| `on-hold` Label | 意図的に保留されている |
| `blocked` Label | 外部要因で進行不可 |
| `DraftIssue` | プロジェクト内メモであり追跡対象外 |

> **Note:** 閾値・除外Labelを変更する場合は、`scripts/detect-stale-items.sh` 内の定数を直接編集してください。

### 出力

#### Workflow Summary（Markdown テーブル）

Status別に滞留Itemの一覧を Markdown テーブル形式で出力します。
...
Status別・担当者別・Label別の集計レポートを生成します。
...
| # | 項目 | 説明 |
|---|------|------|
| 1 | 概要サマリー | 総Item数、Issue/PR 別件数 |
| 2 | Status別件数 | 各Statusの件数と割合（Mermaid 円グラフ付き） |
| 3 | 担当者別件数 | 各担当者のItem数と In Progress / In Review 内訳 |
| 4 | Label別件数 | 各LabelのItem数 |
...
> **Note:** カスタムFieldが設定されていないプロジェクトでは、オプション項目は自動的に非表示となります。

### 出力

#### Workflow Summary（Markdown + Mermaid）

Status別・担当者別・Label別の集計結果を Markdown テーブル形式で出力します。
Status別の分布は Mermaid 円グラフでも可視化されます。

出力項目:

| セクション | 内容 |
|-----------|------|
| Status別 | Status名、件数、割合（テーブル + Mermaid 円グラフ） |
| 担当者別 | 担当者名、件数、In Progress 数、In Review 数 |
| Label別 | Label名、件数 |
| 工数サマリー | Status別の見積もり・実績工数合計（オプション） |
| 期日超過Item | Issue/PR 番号、タイトル、Status、担当者、終了期日、超過日数（オプション） |
...
| 3 | Status別工数 | Statusごとの見積もり・実績工数、消化率 |
| 4 | 乖離Item | 見積もりと実績の乖離が大きいItem一覧（上位 10 件） |
| 5 | 工数未入力Item | 見積もり・実績ともに未入力のItem一覧 |
...
> **Note:** 日付Field（開始予定/実績、終了予定/実績）が設定されていないプロジェクトでは、リードタイム分析は自動的に非表示となります。
...
| 担当者別工数 | 担当者名、Item数、見積もり・実績工数、乖離率（テーブル + Mermaid 円グラフ） |
| Status別工数 | Status名、Item数、見積もり・実績工数、消化率 |
| 乖離Item | Issue/PR 番号、タイトル、担当者、見積もり・実績工数、乖離率 |
| リードタイム分析 | Issue/PR 番号、タイトル、計画・実績日数、乖離日数、日あたり工数（オプション） |
| 工数未入力Item | Issue/PR 番号、タイトル、Status、担当者 |
...
Done StatusのItemを週別に集計し、チームのベロシティ（完了数・完了工数）の推移を可視化します。
...
| セクション | 内容 |
|-----------|------|
| 概要サマリー | 集計期間、Done Item数、平均ベロシティ |
| 週別ベロシティ | 週Label、期間、完了数、完了工数（テーブル + Mermaid 棒グラフ） |
...
> **Note:** この機能はWorkflow④（Project Item エクスポート）から統合されたものです。
...
## ⚠️ Artifactの公開範囲に関する注意事項

このWorkflowで生成されるレポートは `GitHub Actions` の **Artifact** としてアップロードされます。Artifactの公開範囲について、以下の点にご注意ください。

### Artifactのアクセス範囲

- Artifactは **Repositoryの `Actions` タブにアクセスできるユーザーであれば誰でもダウンロード可能** です
- **Public Repositoryの場合、第三者（GitHub アカウントを持つ任意のユーザー）もダウンロード可能です**
- Public RepositoryのForkは Private にできないため、**Fork先でも同様のリスク** があります
- レポートには `PROJECT_PAT` を使って取得した `GitHub Projects` のデータが含まれており、**`GitHub Projects` を直接参照する権限がないユーザーでもレポート経由でデータを閲覧できてしまう可能性** があります

> **Warning:** レポートには担当者名・工数・Statusなどプロジェクト管理上の情報が含まれます。Public Repositoryで運用する場合は、これらの情報が第三者に公開されるリスクを十分にご理解ください。

### Actions のアクセス制限に関する調査結果

GitHub の現行仕様では、**Artifactのダウンロード対象者を特定のユーザーやロールに制限する機能は存在しません**。以下にプラットフォーム側の設定状況と代替手段をまとめます。

#### Repository Settings > Actions > General

| 設定項目 | 制御できること | Artifactへの影響 |
|----------|--------------|----------------------|
| Actions の実行権限 | 許可するアクション・Workflowの制限 | Workflow実行自体は制限可能だが、生成済みArtifactのダウンロードは制限できない |
| Workflow権限（GITHUB_TOKEN） | デフォルトのトークン権限Scope | Artifactのアクセス範囲には影響しない |
...
Organization では Actions の実行ポリシーや GITHUB_TOKEN のデフォルト権限を Organization 全体で強制できますが、**Artifactのダウンロード対象者を制限する設定は Organization レベルでも存在しません**。
...
| 方法 | 概要 | 備考 |
|------|------|------|
| Issue / PR コメントへの投稿 | レポート内容をコメントとして投稿する | Repositoryのアクセス権で制御されるが、Public Repositoryでは閲覧可能 |
| 外部クラウドストレージ（S3, GCS 等） | IAM や署名付き URL で細粒度のアクセス制御が可能 | 追加のインフラ・認証情報管理が必要 |
| GitHub Pages（プライベート公開） | Repository読み取り権限者のみアクセス可能 | GitHub Pro / Team / Enterprise Cloud 等の有料プランが必要（[公式ドキュメント](https://docs.github.com/en/pages/getting-started-with-github-pages/changing-the-visibility-of-your-github-pages-site)） |
| Workflow内での暗号化 | Artifact自体を暗号化し、復号鍵を持つ人のみ閲覧可能にする | 鍵管理の複雑さ、利便性の低下 |
...
.github/workflows/05-analyze-project.yml
  ...
  │   └── scripts/detect-stale-items.sh            # 滞留Item検知
  ├── export-items Job（all or export 選択時）
  │   └── scripts/export-project-items.sh          # Itemエクスポート
...
- [detect-stale-items.sh](../scripts/detect-stale-items) — 滞留Item検知スクリプト
- [export-project-items.sh](../scripts/export-project-items) — Itemエクスポートスクリプト
